...
/** True while background updateVideo may still be filling the DB row. */
...
export function useVideoDetails(videoId?: string) {
  ...
  const { data: extended, mutate: mutateExtended } = useSWR<VideoExtended | null>(
...
export function prefetchVideoBasic(videoId: string) {
  return get({ endpoint: `/public/yt/video?id=${videoId}` });
}
